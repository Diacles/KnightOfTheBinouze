...
public class MainMenuScreen implements Screen {
    ...
    public MainMenuScreen(final KnightOfTheBinouze game) {
        this.game = game;
        ...
        ImageButton kotbButton = createButton("Screen/KnightOfTheBinouze.png", 500, 300);
        ImageButton playButton = createButton("Screen/play_button.png", 150, 150);
        ImageButton saveButton = createButton("Screen/settings_button.png", 120, 120);
        ImageButton exitButton = createButton("Screen/exit_button.png", 70, 70);

        // Ajoute les boutons à la scène
        Table table = new Table();
        table.setFillParent(true);
        table.add(kotbButton).row();
        table.add(playButton).row();
        table.add(saveButton).padBottom(40).row();
    ...
    private void handleButtonClick(String buttonType) {
        ...
        if (buttonType.equals("Screen/KnightOfTheBinouze.png")) {
            menuClick.play();
        }
        else if (buttonType.equals("Screen/play_button.png")) {
            game.setScreen(new GameScreen(game));
            dispose();
        } else if (buttonType.equals("Screen/settings_button.png")) {
            menuClick.play();
            // Ajoutez ici le code pour le bouton "Save"
            //game.setScreen(new SaveMenuScreen(game));
